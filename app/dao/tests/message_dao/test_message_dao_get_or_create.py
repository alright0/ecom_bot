...
class TestMessageDaoGetOrCreate(TestDataBaseClass):
    def setUp(self):
        super().setUp()
    ...
    def test_create_message(self):
        message, created = self.dao.get_or_create(
            self.message_data,
            self.chat,
            self.user,
        )
    ...
    def test_get_message(self):
        self.dao.get_or_create(self.message_data, self.chat, self.user)
        updated_message, created = self.dao.get_or_create(
            self.message_data,
            self.chat,
            self.user,
        )
